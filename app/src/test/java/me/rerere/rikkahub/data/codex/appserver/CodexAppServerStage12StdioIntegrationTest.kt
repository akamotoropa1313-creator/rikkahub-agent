...
class CodexAppServerStage12StdioIntegrationTest {
    ...
    private suspend fun controlled():Fixture{val p=AppServerTestProcess();val m=WorkspaceManager(createTempDirectory("stage12-stdio").toFile(),shellRunner=AppServerRecordingRunner(p));m.ensureWorkspace("workspace");return Fixture(p,WorkspaceCodexAppServerConnectionFactory(m, CodexRuntimeResolver { CodexRuntimeReady("codex", "test", "test", managed = false) }, "0.1.0").create("workspace"))}
