...
class CodexAppServerMcpApiTest {
    ...
    @Test fun `status options typed decoding and forward compatibility`()=runBlocking { fixture().use { f ->
    ...
    @Test fun `missing required status inventory fails`()=runBlocking { fixture().use { f ->
    ...
    @Test fun `resource tool and reload exact wire and results`()=runBlocking { fixture().use { f ->
    ...
    @Test fun `OAuth params and redaction are exact`()=runBlocking { fixture().use { f ->
    ...
    @Test fun `validation and Ready gating prevent writes`()=runBlocking {
        ...
        val t=FakeCodexAppServerTransport();val c=CodexAppServerConnection(CodexAppServerRequestDispatcher(t),CodexAppServerClientInfo("test","1"));val a=CodexAppServerMcpApi(c);assertFails<CodexAppServerNotReadyException>{a.listStatus()};assertFails<CodexAppServerNotReadyException>{a.reload()};assertFails<CodexAppServerNotReadyException>{a.readResource("s","u")};assertFails<CodexAppServerNotReadyException>{a.callTool("t","s","x")};assertFails<CodexAppServerNotReadyException>{a.beginOAuthLogin("s")};assertEquals(0,t.successfulWriteCount());c.close()
    }

    @Test fun `MCP event projection decodes known notifications and malformed diagnostics`()=runBlocking { fixture().use { f ->
